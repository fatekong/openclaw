...
import type { ChannelPlugin, ChannelGatewayContext } from "../../src/types.js";
...
export const demoChannelPlugin: ChannelPlugin<ResolvedDemoAccount> = {
  ...
  gateway: {
    startAccount: async (ctx: ChannelGatewayContext<ResolvedDemoAccount>): Promise<{ stop: () => void }> => {
      ...
      const server = http.createServer(async (req, res) => {
        ...
        if (ctx.onMessage) {
          try {
            ...
            ctx.log?.error?.("onMessage handler threw:", err);
            if (!replied) {
  ...
  outbound: {
    ...
    sendText: async ({ to, text, accountId }) => {
      console.log(`[demo-channel] outbound send to "${to}" (account: ${accountId ?? "default"}): ${text}`);
