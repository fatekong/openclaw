 ...
 * @param cfg         channel 配置（openclaw 中为 OpenClawConfig，本工程为 Record<string,unknown>）
...
export async function startChannelInternal(
  ...
  await Promise.all(
    accountIds.map(async (id) => {
      ...
      const ctx: ChannelGatewayContext = {
        ...
        setStatus: (next: ChannelAccountSnapshot) => {
          console.debug(
            ...
            next.status ?? "(no status)",
          );
        },

        log: {
          info: (...args: unknown[]) =>
            console.log(`[channel:${plugin.id}:${id}]`, ...args),
          warn: (...args: unknown[]) =>
            console.warn(`[channel:${plugin.id}:${id}]`, ...args),
          error: (...args: unknown[]) =>
            console.error(`[channel:${plugin.id}:${id}]`, ...args),
        },
      ...
      const runPromise = startAccount(ctx).then(() => {
